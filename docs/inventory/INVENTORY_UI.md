...
- 게임 데이터: `BNData/`(참조) + `GameData/`(커스텀) 듀얼. `GameDataLoader` + `GameDataJson`(Newtonsoft). `ItemData`에 BN 게임 디테일 통합(description·armor·gun·tool·comestible 등). `GameplayData.GetItem` / `GetMaterial` — 커스텀 우선 → 참조 fallback. **아이템 표시명·설명·레시피 카테고리**는 `item_names.json` (`names` / `descriptions` / `recipe_categories`) + `LocalizationBundle` — [`ITEM_NAMES.md`](ITEM_NAMES.md). BN 재생성: `python Tools/bn_converter/convert.py --bn-path <Cataclysm-BN> --output Assets/StreamingAssets/BNData` (산출에 세 섹션 포함; `--locale-only`는 로케일만). 필드 화이트리스트·미도입 목록: [`docs/equipment/BN_BAKE.md`](../equipment/BN_BAKE.md).
