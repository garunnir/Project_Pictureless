...
| `Assets/Dist/Legacy/Character/ActorSO.cs` | Dialogue System `Actor` 필드에 스킬 인덱스 저장·에디터 UI |
...
| [`ActorSO.cs`](../Character/ActorSO.cs) | `ConstDataTable.Actor.Skill.Active/Passive` 필드 → 카탈로그 인덱스 조회 |
| [`BattleSystem.cs`](../Battle/BattleSystem.cs) | 턴 UI에서 `ActiveSkill.Excute(actor)` 호출 |
| [`ConstDataTable.cs`](../../Scripts/Static/ConstDataTable.cs) | `Skill.Active`, `Skill.Passive` 필드 키 |
...
- [x] `Gameplay/Definitions/Skill/` 스크립트 → `Assets/Dist/Legacy/Skill/` 이동 (2026-07-06)
...
| 날짜 | 내용 |
|------|------|
| 2026-07-06 | 레거시 루트 이동: `Assets/Dist/Scripts/Legacy/` → `Assets/Dist/Legacy/` |
| 2026-07-06 | 폴더 재배치: `SerializedObject/.../Skill` → `Gameplay/Definitions/Skill` → `Legacy/Skill`, `SOData/Skill` → `SOData/Gameplay/Skill`, `Acive` → `Active` |
