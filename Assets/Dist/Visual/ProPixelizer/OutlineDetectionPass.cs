...
namespace Dist.ProPixelizer
{
    ...
    public class OutlineDetectionPass : ProPixelizerPass
    {
        ...
        private static readonly int s_MainTexDepthId = Shader.PropertyToID("_MainTex_Depth");
        private static readonly int s_TexelSizeId = Shader.PropertyToID("_TexelSize");
        private static readonly int s_OutlineBufferId = Shader.PropertyToID(OUTLINE_BUFFER);
        private static readonly int s_PixelScaleId = Shader.PropertyToID("_ProPixelizer_Pixel_Scale");
        private static readonly int s_OutlineDepthTestThresholdId = Shader.PropertyToID("_OutlineDepthTestThreshold");
        private static readonly int s_NormalEdgeDetectionSensitivityId = Shader.PropertyToID("_NormalEdgeDetectionSensitivity");

        private const string k_DrawPassName = "ProPixelizer Outline Objects";
        private const string k_DetectPassName = "ProPixelizer Outline Detection";
        ...
        private sealed class OutlineDrawPassData
        {
        ...
        private sealed class OutlineDetectPassData
        {
        ...
        {
            if (Materials.OutlineDetection == null)
                return;
            ...
            {
                Materials.OutlineDetection.EnableKeyword("DEPTH_TEST_OUTLINES_ON");
                Materials.OutlineDetection.SetFloat(s_OutlineDepthTestThresholdId, DepthTestThreshold);
            }
            else
                Materials.OutlineDetection.DisableKeyword("DEPTH_TEST_OUTLINES_ON");

            if (UseNormalsForEdgeDetection)
                Materials.OutlineDetection.SetFloat(s_NormalEdgeDetectionSensitivityId, NormalEdgeDetectionSensitivity);

            RenderTextureDescriptor camDesc = cameraData.cameraTargetDescriptor;
            var outlineDesc = camDesc;
            outlineDesc.useMipMap = false;
            outlineDesc.autoGenerateMips = false;
            outlineDesc.msaaSamples = 1;
            outlineDesc.depthBufferBits = 0;
            outlineDesc.colorFormat = RenderTextureFormat.ARGB32;
            outlineDesc.graphicsFormat = GraphicsFormat.R8G8B8A8_UNorm;
            ...
            TextureHandle outlineObjectColor = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, outlineDesc, PROPIXELIZER_OBJECT_BUFFER, false, FilterMode.Point);
            TextureHandle outlineObjectDepth = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, depthDesc, PROPIXELIZER_OBJECT_BUFFER + "_Depth", false, FilterMode.Point);
            TextureHandle outlineBuffer = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, outlineDesc, OUTLINE_BUFFER, false, FilterMode.Point);

            ProPixelizerFrameData propData = frameData.GetOrCreate<ProPixelizerFrameData>();
            propData.OutlineObjectColor = outlineObjectColor;
            propData.OutlineObjectDepth = outlineObjectDepth;
            propData.OutlineBuffer = outlineBuffer;

            Vector4 texelSize = new Vector4(
                1f / Mathf.Max(1, camDesc.width),
                1f / Mathf.Max(1, camDesc.height),
                camDesc.width,
                camDesc.height);

            float pixelScale = cameraData.camera.cameraType == CameraType.Preview ? 0.01f : 1f;
            Vector4 renderTargetInfo = GetRenderTargetInfo(cameraData);

            using (var builder = renderGraph.AddRasterRenderPass<OutlineDrawPassData>(k_DrawPassName, out var passData))
            {
                ...
                builder.SetRenderAttachment(outlineObjectColor, 0, AccessFlags.Write);
                builder.SetRenderAttachmentDepth(outlineObjectDepth, AccessFlags.Write);
                builder.AllowGlobalStateModification(true);
                builder.AllowPassCulling(false);

                builder.SetRenderFunc(static (OutlineDrawPassData data, RasterGraphContext ctx) =>
                {
                    ctx.cmd.SetGlobalVector(ProPixelizerRenderTargetInfoId, data.RenderTargetInfo);
                    ctx.cmd.SetGlobalFloat(s_PixelScaleId, data.PixelScale);
                    if (data.UseNormals)
                        ctx.cmd.EnableShaderKeyword("NORMAL_EDGE_DETECTION_ON");
                    else
                        ctx.cmd.DisableShaderKeyword("NORMAL_EDGE_DETECTION_ON");

                    ctx.cmd.ClearRenderTarget(true, true, Color.white);
                    ctx.cmd.DrawRendererList(data.RendererList);
                });
            }

            using (var builder = renderGraph.AddRasterRenderPass<OutlineDetectPassData>(k_DetectPassName, out var passData))
            {
                passData.Material = Materials.OutlineDetection;
                passData.SourceColor = outlineObjectColor;
                passData.SourceDepth = outlineObjectDepth;
                passData.TexelSize = texelSize;

                builder.UseTexture(outlineObjectColor, AccessFlags.Read);
                builder.UseTexture(outlineObjectDepth, AccessFlags.Read);
                builder.SetRenderAttachment(outlineBuffer, 0, AccessFlags.Write);
                builder.SetGlobalTextureAfterPass(outlineBuffer, s_OutlineBufferId);
                builder.AllowGlobalStateModification(true);
                builder.AllowPassCulling(false);

                builder.SetRenderFunc(static (OutlineDetectPassData data, RasterGraphContext ctx) =>
                {
                    data.Material.SetTexture(s_MainTexId, data.SourceColor);
                    data.Material.SetTexture(s_MainTexDepthId, data.SourceDepth);
                    data.Material.SetVector(s_TexelSizeId, data.TexelSize);
                    ctx.cmd.SetGlobalTexture(s_MainTexId, data.SourceColor);
                    ctx.cmd.SetGlobalTexture(s_MainTexDepthId, data.SourceDepth);
                    ctx.cmd.SetGlobalVector(s_TexelSizeId, data.TexelSize);
                    Blitter.BlitTexture(ctx.cmd, data.SourceColor, ScaleBiasIdentity, data.Material, 0);
                });
            }
        }
#endif
