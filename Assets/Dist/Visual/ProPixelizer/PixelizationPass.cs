...
namespace Dist.ProPixelizer
{
    ...
    public class PixelizationPass : ProPixelizerPass
    {
        ...
        private const string k_PixelizationRgPassName = "ProPixelizer Pixelisation RG";

        private static readonly int s_MainTexId = Shader.PropertyToID("_MainTex");
        private static readonly int s_SourceTexId = Shader.PropertyToID("_SourceTex");
        private static readonly int s_DepthId = Shader.PropertyToID("_Depth");
        private static readonly int s_PixelizationMapId = Shader.PropertyToID("_PixelizationMap");
        private static readonly int s_SourceDepthTextureId = Shader.PropertyToID("_SourceDepthTexture");
        private static readonly int s_SceneDepthTextureId = Shader.PropertyToID("_SceneDepthTexture");

        private sealed class PixelizationRgPassData
        {
            public Material PixelizationMapMaterial;
            public Material ApplyPixelizationMap;
            public Material CopyDepth;
            ...
            public TextureHandle OriginalScene;
            public TextureHandle PixelizationMapTexture;
            public TextureHandle PixelatedScene;
            public TextureHandle PixelatedSceneDepth;
            public TextureHandle CameraDepthTemp;
            public TextureHandle SceneDepthUnscaled;
            public bool SkipPreviewDepthRestore;
            public Matrix4x4 ViewMatrix;
            public Matrix4x4 ProjectionMatrix;
            public Vector4 RenderTargetInfo;
            public Vector4 CameraScaleBias;
            public Rect CameraViewport;
            public GlobalKeyword ApplyPixelMapDepthOutputKeyword;
        }

        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
#if DISABLE_PROPIX_PREVIEW_WINDOW
            UniversalCameraData previewCheck = frameData.Get<UniversalCameraData>();
            if (previewCheck.cameraType == CameraType.Preview)
                return;
#endif
            if (!frameData.Contains<ProPixelizerFrameData>())
                return;

            ProPixelizerFrameData propData = frameData.Get<ProPixelizerFrameData>();
            if (!propData.OutlineObjectColor.IsValid() || !propData.OutlineObjectDepth.IsValid())
                return;
            ...
            if (Materials.PixelizationMap == null || Materials.ApplyPixelizationMap == null || Materials.CopyDepth == null)
                return;
            ...
            var mapDesc = camDesc;
            mapDesc.msaaSamples = 1;
            mapDesc.depthBufferBits = 0;
            mapDesc.useMipMap = false;
            mapDesc.autoGenerateMips = false;
            mapDesc.colorFormat = RenderTextureFormat.ARGB32;
            mapDesc.graphicsFormat = GraphicsFormat.R8G8B8A8_UNorm;

            TextureHandle originalScene = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, colorDesc, "ProP_OriginalScene", false, FilterMode.Point);
            TextureHandle pixelizationMap = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, mapDesc, "ProP_PixelizationMap", false, FilterMode.Point);
            TextureHandle pixelatedScene = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, colorDesc, "ProP_PixelatedScene", false, FilterMode.Point);
            TextureHandle pixelatedSceneDepth = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, depthDesc, "ProP_PixelatedScene_Depth", false, FilterMode.Point);
            TextureHandle cameraDepthTemp = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, depthDesc, "ProP_CameraDepthAttachmentTemp", false, FilterMode.Point);
            TextureHandle sceneDepthUnscaled = UniversalRenderer.CreateRenderGraphTexture(
                renderGraph, depthDesc, "ProP_SceneDepthUnscaled", false, FilterMode.Point);

            Vector2 rtScale = RTHandles.rtHandleProperties.rtHandleScale;
            Vector4 cameraScaleBias = new Vector4(rtScale.x, rtScale.y, 0f, 0f);
            var cameraViewport = new Rect(0f, 0f, Mathf.Max(1, camDesc.width), Mathf.Max(1, camDesc.height));

            using (var builder = renderGraph.AddUnsafePass<PixelizationRgPassData>(k_PixelizationRgPassName, out var passData))
            {
                passData.PixelizationMapMaterial = Materials.PixelizationMap;
                ...
                passData.OutlineObjectColor = propData.OutlineObjectColor;
                passData.OutlineObjectDepth = propData.OutlineObjectDepth;
                passData.OriginalScene = originalScene;
                passData.PixelizationMapTexture = pixelizationMap;
                passData.PixelatedScene = pixelatedScene;
                passData.PixelatedSceneDepth = pixelatedSceneDepth;
                passData.CameraDepthTemp = cameraDepthTemp;
                passData.SceneDepthUnscaled = sceneDepthUnscaled;
                passData.SkipPreviewDepthRestore = cameraData.cameraType == CameraType.Preview;
                passData.ViewMatrix = cameraData.GetViewMatrix();
                passData.ProjectionMatrix = cameraData.GetProjectionMatrix();
                passData.RenderTargetInfo = GetRenderTargetInfo(cameraData);
                passData.CameraScaleBias = cameraScaleBias;
                passData.CameraViewport = cameraViewport;
#if BLIT_API
                passData.ApplyPixelMapDepthOutputKeyword = ApplyPixelMapDepthOutputKeyword;
#endif

                builder.UseTexture(cameraColor, AccessFlags.ReadWrite);
                builder.UseTexture(cameraDepth, AccessFlags.ReadWrite);
                builder.UseTexture(propData.OutlineObjectColor, AccessFlags.Read);
                builder.UseTexture(propData.OutlineObjectDepth, AccessFlags.Read);
                builder.UseTexture(originalScene, AccessFlags.ReadWrite);
                builder.UseTexture(pixelizationMap, AccessFlags.ReadWrite);
                builder.UseTexture(pixelatedScene, AccessFlags.ReadWrite);
                builder.UseTexture(pixelatedSceneDepth, AccessFlags.ReadWrite);
                builder.UseTexture(cameraDepthTemp, AccessFlags.ReadWrite);
                builder.UseTexture(sceneDepthUnscaled, AccessFlags.ReadWrite);
                builder.AllowGlobalStateModification(true);
                builder.AllowPassCulling(false);

                builder.SetRenderFunc(static (PixelizationRgPassData data, UnsafeGraphContext context) =>
                {
                    CommandBuffer cmd = CommandBufferHelpers.GetNativeCommandBuffer(context.cmd);
                    cmd.SetGlobalVector(ProPixelizerRenderTargetInfoId, data.RenderTargetInfo);

                    // Viewport-sized copies of scaled camera targets (UV 0-1 for ProPixelizer shaders).
                    context.cmd.SetRenderTarget(data.OriginalScene);
                    Blitter.BlitTexture(cmd, data.CameraColor, data.CameraScaleBias, 0f, false);

                    // CopyDepth frag samples global _Depth (not _BlitTexture); UVs still come from blit scaleBias.
                    cmd.SetGlobalTexture(s_DepthId, data.CameraDepth);
                    context.cmd.SetRenderTarget(data.SceneDepthUnscaled);
                    Blitter.BlitTexture(cmd, data.CameraDepth, data.CameraScaleBias, data.CopyDepth, 0);

                    cmd.SetGlobalTexture(s_MainTexId, data.OutlineObjectColor);
                    cmd.SetGlobalTexture(s_SourceDepthTextureId, data.OutlineObjectDepth);
                    cmd.SetGlobalTexture(s_SceneDepthTextureId, data.SceneDepthUnscaled);
                    context.cmd.SetRenderTarget(data.PixelizationMapTexture);
                    Blitter.BlitTexture(cmd, data.OriginalScene, ScaleBiasIdentity, data.PixelizationMapMaterial, 0);

                    cmd.SetGlobalTexture(s_MainTexId, data.OriginalScene);
                    cmd.SetGlobalTexture(s_PixelizationMapId, data.PixelizationMapTexture);
                    cmd.SetGlobalTexture(s_SceneDepthTextureId, data.SceneDepthUnscaled);
                    cmd.SetGlobalTexture(s_SourceDepthTextureId, data.OutlineObjectDepth);
#if BLIT_API
                    cmd.DisableKeyword(data.ApplyPixelMapDepthOutputKeyword);
#endif
                    context.cmd.SetRenderTarget(data.PixelatedScene);
                    Blitter.BlitTexture(cmd, data.OriginalScene, ScaleBiasIdentity, data.ApplyPixelizationMap, 0);
#if BLIT_API
                    cmd.EnableKeyword(data.ApplyPixelMapDepthOutputKeyword);
#endif
                    context.cmd.SetRenderTarget(data.PixelatedSceneDepth);
                    Blitter.BlitTexture(cmd, data.SceneDepthUnscaled, ScaleBiasIdentity, data.ApplyPixelizationMap, 0);

                    cmd.SetGlobalTexture(s_MainTexId, data.PixelatedScene);
                    cmd.SetGlobalTexture(s_SourceTexId, data.PixelatedScene);
                    cmd.SetGlobalTexture(s_DepthId, data.PixelatedSceneDepth);

                    // Color only: leave camera depth untouched. Mutating scaled camera depth in RG
                    // was wiping scene occlusion (walls/objects appearing transparent).
                    context.cmd.SetRenderTarget(data.CameraColor);
                    cmd.SetViewport(data.CameraViewport);
                    Blitter.BlitTexture(cmd, data.PixelatedScene, ScaleBiasIdentity, 0f, false);
