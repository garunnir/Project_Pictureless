...
public class UICharWindow : MonoBehaviour
{
    ...
    void Start()
    {
        ShowUp(GameManager.Instance.characters[0]);
    ...
    void ShowUp(Garunnir.Character character)
    {
        ...
        foreach (var item in character.field)
        {
            if (!item.Value.Item1) continue;
            var tmp = GameManager.Instance.FormStrDic.Where(x => x.Value == item.Key).ToArray();
            if(tmp==null||tmp.Length==0)continue;
            //첫번쨰부터 검사
            switch (tmp[0].Key.Item1)
            {
                case Form0.image:
                ...
                case Form0.text:
                    ...
                    text.text = UILocalizationManager.instance.textTable.GetFieldText(item.Key) + ": " + item.Value.Item2;
                    text.font = UILocalizationManager.instance.localizedFonts.GetTextMeshProFont(Localization.language);
                    break;
                case Form0.bar:
                    string[] str = item.Value.Item2.ToString().Split('/');
                    ...
                    text1.text = UILocalizationManager.instance.textTable.GetFieldText(item.Key);
                    text1.enabled = true;
