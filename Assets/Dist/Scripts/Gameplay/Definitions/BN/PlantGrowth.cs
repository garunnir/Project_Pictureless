...
namespace Garunnir.Runtime.Gameplay.Data
{
    ...
    public readonly struct PlantGrowthContext
    {
        ...
        public readonly bool GrowthDormant;

        public PlantGrowthContext(
            bool fertilized,
            float weatherGrowFactor,
            bool frostKills,
            bool growthDormant = false)
        {
            Fertilized = fertilized;
            WeatherGrowFactor = weatherGrowFactor;
            FrostKills = frostKills;
            GrowthDormant = growthDormant;
        }
    ...
    public static class PlantGrowth
    {
        ...
        /// <summary>Snow Kind: required grow increased (slower).</summary>
        public const float WeatherSnowGrowFactor = 1.25f;
            ...
            in PlantGrowthContext context)
        {
            if (seed != null && seed.IsTree)
                return ResolveTree(seed, elapsedMinutes, NoFruitHarvestMinute, 0, in context);

            return ResolveField(seed, elapsedMinutes, in context);
        }

        public static PlantGrowthStage ResolveField(
            SeedDetailData seed,
            int elapsedMinutes,
            in PlantGrowthContext context)
        {
        ...
        public static PlantGrowthStage ResolveTree(
            SeedDetailData seed,
            int growthElapsedMinutes,
            int lastFruitHarvestWorldMinute,
            int regrowElapsedMinutes,
            in PlantGrowthContext context)
        {
            float grow = EffectiveGrowMinutes(seed, in context);
            float elapsed = growthElapsedMinutes < 0 ? 0f : growthElapsedMinutes;

            if (elapsed >= grow)
            {
                if (lastFruitHarvestWorldMinute <= NoFruitHarvestMinute)
                    return PlantGrowthStage.Harvestable;

                float regrow = regrowElapsedMinutes < 0 ? 0f : regrowElapsedMinutes;
                return regrow >= seed.FruitRegrowMinutes
