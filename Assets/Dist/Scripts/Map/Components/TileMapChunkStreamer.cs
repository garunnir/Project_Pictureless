...
/// desired = <see cref="CameraGroundView"/> 지면 footprint + <see cref="CameraChunkMargin"/>.
...
public class TileMapChunkStreamer : MonoBehaviour
{
    ...
    public TilePoolStreamEstimate CreatePoolStreamEstimate(IWorldGrid worldGrid)
    {
        ...
        float maxOrtho = CameraGroundView.ResolveOrthographicSize(null, _cinemachineCamera);
    ...
    private void ApplyDesiredChunks()
    {
        ...
        if (CameraGroundView.TryGetFootprintBoundsXZ(
                cam,
                _cinemachineCamera,
                groundPlaneY: 0f,
                out float minX,
                ...
                out float maxZ))
        {
            TileViewportBounds.AppendChunksForWorldBounds(
                _desiredChunks,
                minX,
                maxX,
                minZ,
