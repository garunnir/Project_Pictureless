...
namespace SpriteBaker
{
    ...
    public struct SpriteBakeRequest
    {
        ...
        /// <summary>
        /// Bake-camera elevation above the horizon, in degrees. 0 = side-view
        /// (camera stays level, legacy behaviour). ~30 = common game isometric;
        /// ~35.264 = true isometric. Combined with <see cref="CaptureYawCount"/>
        /// this orbits the camera on a pitched ring so every yaw shares the
        /// same look-down angle — unlike tipping the model via
        /// <see cref="CaptureRotation"/>, which locks the tilt in world space.
        /// Clamped to [0, 89] at bake time.
        /// </summary>
