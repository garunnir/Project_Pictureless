...
namespace SpriteBaker
{
    ...
    /// </list>
    ///
    /// See <c>Assets/Plugins/SpriteBaker/Demo/SpriteBakerDemo.cs</c> for a
    /// copy-paste example.
    /// </summary>
    public class SpriteAtlasBaker : MonoBehaviour
    {
        ...
        private IEnumerator BakeOne(SpriteBakeRequest req)
        {
            ...
            for (int yIdx = 0; yIdx < yawCount; yIdx++)
            {
                ...
                PositionBakeCamera(camGO, bounds, camY, yawDeg, req.CapturePitch);
        ...
        // Pitch > 0 elevates the camera (iso / top-down). Distance is fixed
        // at 10 (orthographic — only affects near/far).
        private static void PositionBakeCamera(GameObject camGO, Bounds bounds, float camY, float yawDeg, float pitchDeg)
        {
            float yawRad = yawDeg * Mathf.Deg2Rad;
            float pitchRad = Mathf.Clamp(pitchDeg, 0f, 89f) * Mathf.Deg2Rad;
